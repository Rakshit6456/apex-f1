...
import { getSeasonSchedule, getRaceResults } from '../utils/f1Api';
import './results.css';
...
function ResultsContent() {
    ...
    useEffect(() => {
        ...
        const fetchResults = async () => {
            setLoading(true);
            const data = await getRaceResults(selectedSeason, selectedRound);
            setRaceData(data);
            setLoading(false);
        };
        fetchResults();
        ...
        <main className="results-container bg-[#0A0A0A] min-h-screen text-white font-sans selection:bg-[#FF1801] selection:text-white">
            ...
            {selectedSeason === 2026 ? (
                <div className="empty-state">
                    ...
                    <h2 className="empty-state-title">Races Yet To Begin</h2>
                    <p className="empty-state-subtitle">The 2026 FIA Formula One World Championship has not started.</p>
                </div>
            ) : (
                <>
                    {/* RACE SELECTOR TABS */}
                    ...
                    {loading ? (
                        ...
                        <div className="empty-state">
                            ...
                            <p className="empty-state-subtitle">We couldn't find results for this race yet.</p>
                        </div>
                    )}
                </>
            )}
